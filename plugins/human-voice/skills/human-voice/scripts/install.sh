#!/usr/bin/env bash
# install.sh — Install ai-lint CLI wrapper to ~/.local/bin/
#
...
case "${1:-}" in
  -h|--help)
    ...
    echo ""
    echo "Creates a local-prefer walker wrapper at ~/.local/bin/ai-lint."
...
EXPECTED_TARGET="$SCRIPT_DIR/ai-lint.sh"
...
if [ "$_DO_CHECK" = true ]; then
  _check_wrapper \
    "ai-lint" \
    ".claude/skills/human-voice/scripts/ai-lint.sh" \
    "bash" \
    "$EXPECTED_TARGET"
  exit 0
fi
